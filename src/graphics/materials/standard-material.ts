...
export class StandardMaterial extends Material {
    ...
    constructor(options: StandardMaterialOptions = {}) {
        ...
        if (options.albedoTexture instanceof Texture) this.albedoTexture = options.albedoTexture;
        else if (typeof options.albedoTexture === "string") this.pendingTextures.albedo = options.albedoTexture;

        if (options.normalTexture instanceof Texture) this.normalTexture = options.normalTexture;
        else if (typeof options.normalTexture === "string") this.pendingTextures.normal = options.normalTexture;

        if (options.roughnessTexture instanceof Texture) this.roughnessTexture = options.roughnessTexture;
        else if (typeof options.roughnessTexture === "string") this.pendingTextures.roughness = options.roughnessTexture;

        if (options.metallicTexture instanceof Texture) this.metallicTexture = options.metallicTexture;
        else if (typeof options.metallicTexture === "string") this.pendingTextures.metallic = options.metallicTexture;

        if (options.aoTexture instanceof Texture) this.aoTexture = options.aoTexture;
        else if (typeof options.aoTexture === "string") this.pendingTextures.ao = options.aoTexture;

        if (options.ormTexture instanceof Texture) this.ormTexture = options.ormTexture;
        else if (typeof options.ormTexture === "string") this.pendingTextures.orm = options.ormTexture;
    ...
    public getBindGroup(ctx: Context): GPUBindGroup {
        if (Object.keys(this.pendingTextures).length > 0) {
            if (this.pendingTextures.albedo) this.albedoTexture = Texture.loadBackground(ctx, this.pendingTextures.albedo);
            if (this.pendingTextures.normal) this.normalTexture = Texture.loadBackground(ctx, this.pendingTextures.normal);
            if (this.pendingTextures.roughness) this.roughnessTexture = Texture.loadBackground(ctx, this.pendingTextures.roughness);
            if (this.pendingTextures.metallic) this.metallicTexture = Texture.loadBackground(ctx, this.pendingTextures.metallic);
            if (this.pendingTextures.ao) this.aoTexture = Texture.loadBackground(ctx, this.pendingTextures.ao);
            if (this.pendingTextures.orm) this.ormTexture = Texture.loadBackground(ctx, this.pendingTextures.orm);
        ...
        if (this.isDirty) {
            ...
            ctx.device.queue.writeBuffer(this.uniformBuffer, 0, this.bufferData.buffer);
            this.isDirty = false;
        }

        // Gather all textures. If one is missing, provide a dummy texture so WebGPU bind group gets created successfully
        let tAlbedo = this.albedoTexture || Texture.getDummyWhite(ctx);
        let tNormal = this.normalTexture || Texture.getDummyNormal(ctx);
        let tRoughness = this.roughnessTexture || Texture.getDummyWhite(ctx);
        let tMetallic = this.metallicTexture || Texture.getDummyWhite(ctx);
        let tAO = this.aoTexture || Texture.getDummyWhite(ctx);
        let tORM = this.ormTexture || Texture.getDummyWhite(ctx);

        // To avoid constantly remaking bind group, we do it if not exists or if a texture finishes loading
        let needsRebuild = !this.bindGroup;
