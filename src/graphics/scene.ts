...
export class Scene extends Node {
  ...
  public addInstance(
    ...
    // Função auxiliar para clonar recursivamente o nó e suas sub-meshes
    const cloneNode = (source: Node3D | Mesh): Node3D | Mesh => {
      let clonedNode: Node3D | Mesh;

      if (source instanceof Mesh) {
        // Se for uma Mesh, criamos uma nova compartilhando a geometria original (Instanciação)
        clonedNode = new Mesh(this.ctx, {
          geometry: source.geometry,
          // Usa o material customizado se passado nas opções superiores, caso contrário mantém o do sub-mesh
          material: parsedOptions.material || source.material,
        });
      } else {
        // Se for um nó de agrupamento genérico (Node3D)
        clonedNode = new Node3D(); // Ou como você instanciar o seu Node3D base
      }

      // Copia transformações locais básicas do original (posição, rota, escala)
      ...
      for (const child of source.children) {
        if (child instanceof Node3D || child instanceof Mesh) {
          ...
          clonedNode.add(clonedChild); // Adiciona o filho clonado ao nó pai clonado
        }
      }
    ...
    // Cria a árvore clonada/instanciada a partir do template
    const finalClone = cloneNode(template) as Mesh;

    // Aplica as opções de transformação globais passadas para a instância no nó raiz
